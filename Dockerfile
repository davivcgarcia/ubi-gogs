...
ENV GOGS_PATH ${GOGS_BASEDIR}/gogs
ENV GOGS_DATADIR ${GOGS_PATH}/data
ENV GOGS_PORT 3000
ENV GOGS_USER git
...
RUN yum -y install git nss_wrapper gettext && \
    ...
    mkdir ${GOGS_DATADIR} && \
    chgrp -R 0 ${GOGS_HOMEDIR} ${GOGS_PATH} ${GOGS_DATADIR} && \
    chmod -R g+rwX ${GOGS_HOMEDIR} ${GOGS_PATH} ${GOGS_DATADIR} && \
    cd ${GOGS_PATH}
